...
export function ProviderModelConfiguration() {
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try {
          ...
          const mergedProviders = providersList.map((originalProvider) => {
            ...
            if (savedProvider) {
              return {
                ...
                models: originalProvider.models.map((originalModel) => {
                  const savedModel = savedProvider.models.find(
                    (m: Model) => m.id === originalModel.id
                  );
                  if (savedModel) {
                    return {
                      ...originalModel,
                      ...
                      inputCost:
                        savedModel.inputCost ?? originalModel.inputCost,
                      outputCost:
                        savedModel.outputCost ?? originalModel.outputCost,
                      customCost: savedModel.customCost ?? false,
                    };
                  }
                  return {
                    ...originalModel,
            ...
            return {
              ...originalProvider,
              models: originalProvider.models.map((model) => ({
                ...model,
                enabled: true,
              })),
            };
        ...
        if (config.defaultModels) setDefaultModels(config.defaultModels);
      } catch (error) {
        console.error("Failed to load saved configuration:", error);
      }
    } else {
      // Initialize models with enabled flag if no saved config
      setProviders(
  ...
  }, [providers, defaultProvider, defaultModels]);
              ...
              >
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    >
                      {provider.icon && (
                        <provider.icon className="size-3.5 text-white" />
                      )}
                    </div>
